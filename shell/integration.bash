...
#
#   echo '[ -f ~/.cache/terminal-dev/shell/integration.bash ] && \
#     . ~/.cache/terminal-dev/shell/integration.bash' >> ~/.bash_profile
#
...
_termdev_preexec() {
  ...
  case "$BASH_COMMAND" in
    _termdev_precmd*) return ;;
  esac
  # 한 줄에 여러 명령이 있어도 C는 처음 한 번만
  [ -n "$_termdev_executing" ] && return
  printf '\033]133;C\007'
  _termdev_executing=1
}

trap '_termdev_preexec' DEBUG

# 기존 PROMPT_COMMAND보다 먼저 실행되어야 $?가 사용자 명령의 것이다.
if [ -n "$PROMPT_COMMAND" ]; then
  PROMPT_COMMAND="_termdev_precmd; $PROMPT_COMMAND"
else
  PROMPT_COMMAND="_termdev_precmd"
fi
