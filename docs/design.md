...
- [x] Phase 21 — 퀵윈 묶음 (2026-09-01): 폰트 크기 단축키 · SUN_LEN 패닉 수정 ·
  ...
  - 테스트 18개 추가 (keybind 파싱·config 범위 검사·외양별 프리셋·shell_quote·
    소켓 경로 폴백)
- [x] Phase 22 — 에이전트 오케스트레이션 CLI + 탭 상태 대시보드 (2026-09-08)
  - **`eden` 서브커맨드** — `list [--json]` · `new [--cwd]` · `send <id> [텍스트]
    [-n|-r|-w --timeout]` · `wait <id>` · `capture <id> [-s] [-n N]` · `kill <id>`.
    tmux의 `list-windows / send-keys / capture-pane`에 해당한다. 같은 바이너리가
    첫 인자로 GUI/CLI를 가르고, 모르는 인자는 GUI로 넘긴다 (Finder 인자 등).
    Homebrew cask에 `binary` 스탠자를 추가해 `eden`이 PATH에 걸린다
  - **mux 프레임 다섯 개** — `SEND · PEEK · LIST_INFO · SPAWN · KILL_ID`. 전부
    "구독 없이 한 번 묻고 끊는" 단발 요청이라 기존 attach 경로와 섞이지 않는다.
    구버전 데몬은 모르는 태그에 응답 없이 연결을 닫으므로 EOF를 "구버전 데몬"
    오류로 바꿔 준다. `SESSION_INFO` 레코드에는 길이를 붙여 꼬리 필드 추가가
    하위호환이다
  - **데몬이 셸 상태를 안다** — 출력 바이트에서 OSC 133/7을 스캔해 세션마다
    running · last_exit · cwd · 명령줄 · 시작/완료 카운터를 유지한다
    (`osc::ShellState`). GUI의 스캐너와 같은 함수를 쓰도록 `osc.rs`로 뽑았다.
    셸 통합 스크립트는 fish 관례를 따라 `C;cmdline_url=<퍼센트 인코딩>`으로
    명령줄을 보낸다 (zsh·bash 모두, 512바이트 절단)
  - **`send --wait`는 카운터로 판정한다** — running 플래그를 폴링하면 `false`처럼
    짧은 명령은 시작과 끝을 둘 다 놓친다. 보내기 전 완료 수를 잡고 "늘었는가"로
    본다. 2초 안에 시작 수가 안 늘면 명령이 아니었던 것(빈 줄·TUI 내부 입력)
    으로 보고 0
  - **`capture`는 헤드리스 Term** — 리플레이를 세션과 같은 크기의 Term에 재생해
    `bounds_to_string`으로 뽑는다. GUI가 붙을 때와 같은 경로라 화면이 같다
  - **탭 상태 점** — 탭 제목 앞에 실행 중(파랑)·미확인 성공(초록)·미확인
    실패(빨강) 점. "보고 있지 않을 때 끝났다"는 알림과 같은 조건이지만 임계
    시간이 없다. 활성 탭이 그려지면 지워진다 — "그려졌다 = 봤다"로 두면 탭
    전환 경로가 몇 개든 한 곳에서 끝난다
  - **고아 세션 자동 부착** — 창 포커스 때 데몬 세션 목록과 탭을 대조해
    `eden new`로 만든 세션을 탭으로 붙인다. LIST 한 번이라 값싸다
  - **수동 검증에서 잡힌 버그** — 데몬이 CLI의 stdout 파이프를 물려받아
    `id=$(eden new)`가 영영 안 끝났다. spawn 시 표준 입출력을 `/dev/null`로
  - 테스트 24개 추가 (OSC 파싱·ShellState·SessionInfo 인코딩·CLI 표/JSON/
    이스케이프/렌더)
...
- ~~`eden` CLI 서브커맨드 (`list` / `send` / `capture` / `wait` / `new` / `kill`)~~
  — **Phase 22 완료**. 터미널 안에서 attach하는 `eden attach`(tmux attach식
  raw 모드 클라이언트)는 남았다 — D의 원격 mux와 묶인다
- ~~에이전트 세션 대시보드~~ — **Phase 22 완료** (탭 바 상태 점). 상태바에
  전체 요약("3 running")을 띄우는 것은 남았다
...
남은 후보 중 다음 타로 유력한 것은 **B(블록 접기·재실행)** 다. Phase 22가
C의 뼈대(CLI + 상태 추적)를 놓았으니, B의 블록 재실행은 이제 `cmdline_url`로
명령줄을 알 수 있어 절반이 준비된 셈이다. C의 나머지(git worktree 병렬 세션,
에러 설명 모드)는 CLI 위에 스크립트로 먼저 굴려 보고 내장 여부를 정한다.
